...
cargo run --release -p vectro_cli -- compress sample.jsonl dataset.bin 2>/dev/null
...
cargo run --release -p vectro_cli -- compress sample.jsonl dataset_q.bin --quantize 2>/dev/null
...
cargo run --release -p vectro_cli -- search "0.9,0.1,0.2,0.3,0.4" --top-k 3 --dataset dataset.bin 2>/dev/null
echo ""
sleep 1

echo "Query 2: Search for 'car' (0.1, 0.9, 0.8, 0.7, 0.6)"
cargo run --release -p vectro_cli -- search "0.1,0.9,0.8,0.7,0.6" --top-k 3 --dataset dataset.bin 2>/dev/null
echo ""
sleep 1

echo "Query 3: Search for 'red' (0.5, 0.5, 0.1, 0.1, 0.9)"
cargo run --release -p vectro_cli -- search "0.5,0.5,0.1,0.1,0.9" --top-k 3 --dataset dataset.bin 2>/dev/null
echo ""
sleep 1
